...
class Database {
  ...
  void addUser(UserCredential cred, String nickname) {
    ...
    db
        .collection('users')
        .doc(cred.user!.uid)
        .set({'ownedCosmetic': cosmeticList});
    db
        .collection('users')
        .doc(cred.user!.uid)
        .set({'shopCharacter': characterShop});
    db
        .collection('users')
        .doc(cred.user!.uid)
        .set({'shopCosmetic': cosmeticShop});
    db
        .collection('users')
        .doc(cred.user!.uid)
  ...
  getCharacterForShop(String userUid) async {
    List<Character> characters = [];
    var docSnap = await db.collection('users').doc(userUid).get();
    var data = docSnap.data();
  ...
  getCosmeticFromUser(String userUid) async {
  ...
  getCosmeticForShop(String userUid) async {
    List<Cosmetics> cosmetics = [];
    var docSnap = await db.collection('users').doc(userUid).get();
    var data = docSnap.data();
    List<int> cosmeticList = data!["shopCosmetic"].cast<int>();
    for (int i in cosmeticList) {
      cosmetics.add(_loader.cosmeticList[i]);//not that list
    }
    return cosmetics;
  }
